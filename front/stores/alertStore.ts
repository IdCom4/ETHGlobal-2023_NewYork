...
import { AlertModes, AlertStatuses, DEFAULT_REQUEST_ERROR_MESSAGE, DEFAULT_REQUEST_SUCCESS_MESSAGE } from '@/assets/ts/enums'
...
export const useAlertStore = defineStore({
  ...
  actions: {
     ...
     */
    sendAlert(status: AlertStatuses, message: string, persistant = false, durationInMs = 5000) {
      this.alert = {
        status,
        message,
        persistant,
     ...
     */
    handleRequestResult(alertControl?: IAlertControl | null, error?: IRequestError | null, data?: unknown | null) {
      // no alert required
      ...
      const dataSuccessMessage = data && typeof data === 'object' ? (data as Record<string, string>).message : null

      const status = error ? AlertStatuses.ERROR : AlertStatuses.SUCCESS
      const message = error
        ? alertControl.errorMsg || error.message || DEFAULT_REQUEST_ERROR_MESSAGE
        : alertControl.successMsg || dataSuccessMessage || DEFAULT_REQUEST_SUCCESS_MESSAGE
