...
import { GEOLOCALISATON_TYPES, InputTypes, KeyNames, SelectOptionStates } from '@/types/constants'
import { Address } from '@/assets/ts/classes/api-models'
 ...
 * @property { String } geolocalisationType - Le type de géolocalisation pour l'input. Par défaut : 'address'
 */
...
const props = defineProps({
  ...
  modelValue: {
    type: [String, Number, Boolean, Object, null] as PropType<string | number | boolean | IAddress | object | null>,
    default: null
  },
  icon: {
    type: [Array, String],
  ...
  /* ADDRESS SPECIFIC */
  geolocalisationType: {
    type: String as PropType<GEOLOCALISATON_TYPE>,
    default: GEOLOCALISATON_TYPES.ADDRESS
  },
  /* SELECT SPECIFIC */
  selectOptions: {
...
const isPreciseAddressRequested = computed(() => props.geolocalisationType === GEOLOCALISATON_TYPES.ADDRESS)

/* >=========== TYPE CHECK */
// check if an object is a semi strict IAddress
const isIAddress = (object: unknown): object is IAddress => {
  if (!object || typeof object !== "object") return false
  if (!("street" in object)) return false
  if (!("city" in object)) return false
  if (!("zipCode" in object)) return false
  return true
}
...
const getValueAsPrintableString = (value: unknown): string => {
  ...
  if (props.type === InputTypes.ADDRESS && isIAddress(value)) return new Address(value).toString()
  else if (props.type === InputTypes.SELECT) return updatableSelectOptions.value.find((option) => JSON.stringify(option.value) === JSON.stringify(value))?.display || ''
...
/* ADDRESS */
const handleAddressValueUpdate = (target: HTMLInputElement) => {
  // update input value
  inputCurrentTextualValue.value = target.value
  // fetch geocoder suggestions
  useGeocoder().getAddressSuggestionsFromGeocoderAPI(target.value, isPreciseAddressRequested.value, props.geolocalisationType).then((suggestionsData) => {
    updatableSelectOptions.value = suggestionsData.map((element) => ({ value: element, display: element.place_name }))
  })
}
...
const dispatchValueUpdateHandling = (event: Event) => {
  ...
  if (props.type === InputTypes.ADDRESS)
    handleAddressValueUpdate(target)
  else if (props.type === InputTypes.CHECKBOX || props.type === InputTypes.TOGGLE) {
...
const emitSelectedOption = (option: IInputSelectOptions) => {
  // if input TYPE is an address, cast the option to an address and reset the options
  if (props.type === InputTypes.ADDRESS && option.value instanceof MapboxFeature) {
    emitEvent('update:modelValue', isPreciseAddressRequested ? option.value.toStrictAddress() : option.value.toAddress())
    updatableSelectOptions.value.length = 0
  }
  // else simply emit the option value and close the dropdown
  else {
