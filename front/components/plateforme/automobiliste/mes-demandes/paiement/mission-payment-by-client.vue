...
const props = defineProps({ missionId: { type: String, default: '' }, professionalId: { type: String, default: '' } })
...
const mission = ref<IMission | null>(null)
...
const fetchMission = async () => {
  const response = await useAPI().missions.getClientMissionById(props.missionId, { mode: AlertModes.ON_ERROR })
...
watch(mission, () => {
  if (mission.value) {
    const professionalEntry = mission.value.professionalEntries.find((entry) => entry.professional._id === props.professionalId)
    const proposal = professionalEntry?.proposal

    if (!proposal) return

    totalTTC.value = proposal.quote.totalTTCToClient
  }
})
