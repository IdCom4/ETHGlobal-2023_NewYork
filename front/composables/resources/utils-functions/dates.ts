...
class DatesUtils {
  ...
  static getDateData(date: Date, pretty = false): IDateData {
  ...
  static getStrDateData(strDate: string, format = 'dd/MM/yyyy HH:mm', castToInt = false): IDateData | null {
    ...
    for (let i = 0; i < __dateParsingProperties.length; i++) {
      ...
      if (dataIndex >= 0) {
        ...
        if (!dateData || isNaN(intData)) {
          return null
        }

        parsedData[__dateParsingProperties[i].label] = castToInt ? intData : dateData
      }
  ...
  static getDateFromStr(strDate: string, format = 'dd/MM/yyyy HH:mm'): Date | null {
    const dateData = this.getStrDateData(strDate, format)
  ...
  static formatStrDate(strDate: string, formatFrom: string, formatTo: string): string | null {
  ...
  static getDiffTimeInSeconds(date1: Date, date2: Date) {
  ...
  static formatDateRangeToPrintableString(dateRange: IDateRange | IFlexibleDateRange, format: string): string {
    const startFormatted = this.getDateFromStr(dateRange.begin, format)
    if (!startFormatted) return '-'

    const startMonthYear = startFormatted.toLocaleString('default', { month: 'short', year: 'numeric' })
    if (!dateRange.end) return `${startMonthYear} - Aujourd'hui`

    const endFormatted = this.getDateFromStr(dateRange.end, format)
    if (!endFormatted) return '-'

    const endMonthYear = endFormatted.toLocaleString('default', { month: 'short', year: 'numeric' })
