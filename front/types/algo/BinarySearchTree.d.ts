...
export class BinarySearchTree<T> {
   ...
   * @param [Function | null]   compareNodesValue   A optional function that must take the data of 2 nodes as parameter and compare them as see fit. Must return < 0 if first is lesser, > 0 if greater, 0 if equal
   * @param [Function | null]   nodeValueToString   A optional function that must take the data of a node as parameter and  returns it string representation
   */
  // eslint-disable-next-line prettier/prettier
  constructor(arrayOfData: T[] | null, compareNodesValue?: (data1: T, data2: T) => number | null, nodeValueToString?: (data: T) => string | null) {
   ...
   * @param {BSTNode<T> | null} root  The root of the tree we want to print. If not specified, this BST's top root is used
   */
  print(depth = 0, root: BSTNode<T> | null = null): void {
    const rootToPrint = root || this.root
