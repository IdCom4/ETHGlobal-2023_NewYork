...
export class UpdateVehicleInvoiceRequest {
  ...
  @ApiProperty({
    description: "Fichier de la facture. Remplace le fichier actuel s'il est fourni.",
  ...
  @ApiProperty({ description: 'Montant total de la facture toutes taxes comprises', example: 120, required: false })
  @IsOptional()
  @IsPositive({ message: 'Le montant total TTC doit être positif' })
  totalTTC?: number

  @ApiProperty({ description: 'Id de la voiture liée à la facture', example: '912a5b3f6c9e0d401f8e7d2c', required: false })
  @IsOptional()
  @IsMongooseId({ message: "L'id du véhicule est invalide" })
  vehicleId?: string

  @ApiProperty({ description: 'Date de création de la facture', example: '11/05/2020 00:00', required: false })
  @IsOptional()
  @ParseStringDate('dd/MM/yyyy HH:mm', { message: 'La date de facturation est invalide' })
  madeAt?: Date

  @ApiProperty({ description: 'Kilométrage du véhicule', example: 98000, required: false })
  @IsOptional()
